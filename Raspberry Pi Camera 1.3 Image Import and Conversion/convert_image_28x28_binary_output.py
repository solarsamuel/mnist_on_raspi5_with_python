...
for i in range(28):
    for j in range(28):
        downsampled_image_array[i, j] = 1 if np.max(image_array[i*2:i*2+2, j*2:j*2+2]) > 128 else 0
...
np.savetxt('snapshot_binary.csv', downsampled_image_array, fmt='%d', delimiter=',')

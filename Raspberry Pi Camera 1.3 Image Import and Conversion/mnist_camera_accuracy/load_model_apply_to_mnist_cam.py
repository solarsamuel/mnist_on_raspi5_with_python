...
data = pd.read_csv('/home/pi/Desktop/mnist_cam.csv')
...
def init_params_from_csv_W1():
    ...
    W1_data = []
    with open('W1_output94.csv', 'r') as file:
        for line in file:
...
def init_params_from_csv_W2():
    # Load weights from CSV files
    weights_data = {}
    with open('W2_output94.csv', 'r') as file:
        current_neuron = None
        for line in file:
...
def init_params_from_csv_b1():
    ...
    biases1_data = []
    with open('b1_output94.csv', 'r') as file:
        for line in file:
...
def init_params_from_csv_b2():
    # Load biases from CSV file
    biases2_data = []
    with open('b2_output94.csv', 'r') as file:
        for line in file:
            if line.strip().startswith('Biases'):
                continue  # Skip the line if it starts with 'Biases'
